...
const float spotLightStrength = 0.4;
...
vec3 pointLight() {
 ...
 vec3 color = vec3(0,1,0); //Green Light
...
vec3 spotLight()
{
 ...
 vec3 direction = vec3(0,.5,1);
 vec3 lightColor = vec3(0,1,1); //Blue Light
 ...
 float theta = dot(lightDir, normalize(-direction));
  if(theta > cutoff) 
  {
   ...
   return (ambient + diffuse + specular) * spotLightStrength;
  } 
  else 
  {
      return ambient * spotLightStrength;
  }
}

vec3 infiniteLight() {
 vec3 lightColor = vec3(1,0,0); //red light
 ...
 vec3 infiniteLight = (ambient + diffuse + specular) * infiniteStrength;
...
void main() {
 ...
 vec3 spot = spotLight();
 vec3 ambient = ambientLight();
 outColor = glColor * (infinite+spot+point);
}