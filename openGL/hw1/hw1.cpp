...
*   after that use "MESA_GL_VERSION_OVERRIDE=4.5 ./hw1" to run program
**/
#include <iostream>
...
vector<GLfloat> loadVector(void) {
    int colorPos = 0;
    GLfloat square_size = 0.0625; //Size needed to display a 32x32 image
    vector<GLfloat> points;
    
    //Fill "points" with GLfloats corresponding to the proper location and color
    for (GLfloat y = 1.0; y > -1.0; y = y - square_size) {
        for (GLfloat x = -1.0; x < 1.0; x = x + square_size) {
            ...
            for (int i = 0; i < 30; i++){
                points.push_back(vertices[i]); //adds the floats into the vector
            }
            colorPos++;
        }
    } 
